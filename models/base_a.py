...
# TO DO : FIX ATTENTION LAYER, FREEZE EMBEDDINGS
class BaseA(nn.Module):
    '''
    Content Attention Module
    
    Attention weights of each memory slice m_i is calculated with a FwNN2
    c_i = W_1 tanh(W2_m_i + W_3v_a + b_1)
    '''
    ...
    def forward(self, inputs):
        ...
        for _ in range(self.opt.hops): 
            x = self.x_linear(x)
            v_ns = self.attention(memory, x)              # aspect-specific sentence representation (embedding)
